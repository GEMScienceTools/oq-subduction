...
class CrossSection:
    ...
    def _set_vertexes(self):
        #adjusting set vertices so they extend "backwards" for 15% of the profile length
        self.plo.append(self.olo)
        self.pla.append(self.ola)
        for lngh, strk in zip(self.length, self.strike):
        #    tlo1, tla1 = point_at(self.olo, self.ola, strk-180, lngh*0.15)
           # self.plo.append(tlo1)
           # self.pla.append(tla1)
           # tlo, tla = point_at(self.olo, self.ola, strk, lngh)
